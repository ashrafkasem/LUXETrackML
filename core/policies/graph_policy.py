...
def save_graph(graph, filename):
    ...
    # graph_to_sparse function sparsifies to graph so that it can be stored with less memory
    # the new convention builds graphs with sparse arrays so these functions are not really necessary anymore
    # I am still leaving them here just in case
    # they might be deleted later
    #np.savez(filename, **graph_to_sparse(graph))
    np.savez(filename, graph)
